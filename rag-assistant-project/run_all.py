#!/usr/bin/env python3
"""Run the whole project with one command and see the output only.

    python run_all.py                  # notebook -> tests -> backend + frontend
    python run_all.py --install        # pip install every dependency first
    python run_all.py --skip-notebook --skip-tests    # just start the app
    python run_all.py --no-serve       # only the notebook and the tests

Steps:
  1. Checks that Ollama is running and the model is pulled.
  2. Runs the notebook top to bottom, prints ONLY its outputs (no code) and writes an
     output-only report to notebooks/rag_pipeline_output.html.
  3. Runs the backend tests (pytest).
  4. Starts the FastAPI backend and the Streamlit frontend and opens the browser.
"""
...
PACKAGES = {
    ...
    "fastapi": "fastapi", "httpx": "httpx", "pytest": "pytest", "pydantic_settings": "pydantic-settings",
...
NEEDS = {
    ...
    "tests": ["fastapi", "httpx", "pytest", "pydantic_settings", "chromadb", "ollama"],
...
def missing_packages(steps: list[str]) -> list[str]:
    ...
    return sorted(PACKAGES[m] for m in needed if importlib.util.find_spec(m) is None)
...
def run_notebook() -> None:
    banner("1/3  Running the notebook (top to bottom)")
    run = lambda cmd: subprocess.run(cmd, cwd=NOTEBOOK_DIR, check=True)
    try:
...
def run_tests() -> bool:
    banner("2/3  Running the backend tests")
    return subprocess.run([PY, "-m", "pytest", "-q"], cwd=BACKEND).returncode == 0
...
def serve() -> None:
    banner("3/3  Starting the backend and the frontend")
    ensure_env_files()
...
def main() -> None:
    ...
    steps = [s for s, skip in (("notebook", args.skip_notebook), ("tests", args.skip_tests),
                               ("serve", args.no_serve)) if not skip]
    if not steps:
        print("Nothing to do.")
        return
    ...
    if "serve" in steps:
        serve()
    elif "tests" in steps and not tests_ok:
        raise SystemExit(1)
